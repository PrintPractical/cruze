...
3. **Design the walking skeleton.** Run `cruze new change walking-skeleton --title "Walking skeleton"`. Then:
   ...
   - Choose the thinnest end-to-end slice that crosses every layer: one real inbound adapter, one use case, the domain it needs, one driven adapter and the composition root. It also makes CI real: format, lint, build, test, `cruze check --ci` and `cruze trace --all`. CI runs the CLI with `npx --yes @printpractical/cruze@<version>`, pinned to the version `cruze --version` reports. `plan` fills `commands:` in `.cruze/config.yaml` for it.
   - Write its `## Intent`.
