...
- The design workflow: `explore` (optional), `envision` for the what, `architect` for the how, and `roadmap` for the order of work.
...
3. Explain the design workflow: `envision` then `architect` at project scope for a new project, then `envision "<feature>"` and `architect "<feature>"` for each feature. The delivery skills (plan, build, verify, land) arrive in a later release.
