...
1. Publish the first version by hand, because npm only accepts a trusted publisher for a package that already exists. From a clean checkout of `main`, run `npm login`, then `npm publish --access public`.
...
The workflow runs `npm run check`, confirms the tag matches `package.json`, and publishes with provenance.
